...
"""
  ...
  def make_adjacency_matrix(self):
    ...
    # create an adjacency matrix of degree 2
    self.matrix_2 = np.matmul(self.adjacency_matrix, self.adjacency_matrix)

  def label_greedy(self):
    """
    Compute the L(2,1) label for each vertex in the graph using a greedy algorithm
    """
    # iterate through the vertices
    for i in range(len(self.vertices)):
      # initialize the labeling number to 0
      label_number = 0
      for j in range(i):
        while (self.adjacency_matrix[i][j] > 0 and abs(label_number - self.greedy_labels[j]) < 2) or (self.matrix_2[i][j] > 0 and abs(label_number - self.greedy_labels[j]) < 1):
          # increment the labeling number if the current one violates the L(2,1) labeling rules
          label_number += 1
      self.greedy_labels.append(label_number)

  def label_chang_kuo_mod(self):
  """
  Compute the L(2,1) label for each vertex in the graph using the modified Chang-Kuo algorithm
  """
